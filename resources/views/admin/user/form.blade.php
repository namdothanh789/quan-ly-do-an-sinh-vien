                    ...
                    <div class="card-body">
                        <div class="tab-content">
                            <div class="tab-pane active" id="settings">
                                ...
                                <div class="form-group row" >
                                    <label for="inputName2" class="col-sm-2 col-form-label">Chức vụ <sup class="title-sup">(*)</sup></label>
                                    <div class="col-sm-10">
                                        <select name="position_id" class="form-control">
                                            <option value="">Chọn chức vụ</option>
                                            @foreach($positions as $key => $position)
                                                <option  {{old('position_id', isset($user) ? $user->position_id : '') == $position->id ? 'selected=selected' : '' }}  value="{{$position->id}}">{{$position->p_name}}</option>
                                            @endforeach
                                        </select>
                                        <span class="text-danger"><p class="mg-t-5">{{ $errors->first('position_id') }}</p></span>
                                    </div>
                                </div>

                                <div class="form-group row" >
                                    <label for="inputName2" class="col-sm-2 col-form-label">Khoa / Bộ môn <sup class="title-sup">(*)</sup></label>
                                    <div class="col-sm-10">
                                        <select name="department_id" class="form-control">
                                            <option value="">Chọn khoa / bộ môn</option>
                                            @foreach($departments as $key => $department)
                                                <option  {{old('department_id', isset($user) ? $user->department_id : '') == $department->id ? 'selected=selected' : '' }}  value="{{$department->id}}">{{$department->dp_name}}</option>
                                            @endforeach
                                        </select>
                                        <span class="text-danger"><p class="mg-t-5">{{ $errors->first('department_id') }}</p></span>
                                    </div>
                                </div>
                                <div class="form-group row" >
